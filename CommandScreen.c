...
static int CommandScreen_scanWide(InfoScreen* this, const char* p, size_t total, char* line) {
   ...
   for (size_t i = 0; i < total; ) {
      assert(line_offset >= 0 && (size_t)line_offset < MAX_LINE_SIZE);
      wchar_t wc;
      ...
      } else if (line_cols > COLS) {  // got a wide char, wrap by previous one
...
static int CommandScreen_scanAscii(InfoScreen* this, const char* p, size_t total, char* line) {
   int line_offset = 0, line_size = 0, last_spc = -1;
   for (size_t i = 0; i < total; i++, line_offset++) {
      ...
      if (line_offset == COLS) {
         line_size = (last_spc == -1) ? line_offset : last_spc;
         line[line_size] = '\0';
         InfoScreen_addLine(this, line);
...
static void CommandScreen_scan(InfoScreen* this) {
   ...
   Panel_setSelected(panel, idx < 0 ? 0 : idx);
}
